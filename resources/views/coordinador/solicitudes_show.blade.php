                        ...
                        <tr class="p-0" style="font-size: 13px;">
                            <form action="{{route('coordinador.solicitud.update', $solicitud)}}" method="POST">
                                ...
                                @foreach ($students as $student)
                                    ...
                                    @endif
                                @endforeach
                                <td style="text-transform: capitalize;" class="text-warning p-1">{{$solicitud->solicitud_status}}</td>
                                <td class="p-1"><button class="btn btn-success px-1 py-0" style="font-size: 13px;">Aprobar</button><a class="ml-1 btn btn-primary px-1 py-0" style="font-size: 13px;" href="{{route($solicitud->solicitud_view, $solicitud)}}" target='_blank'>Imprimir</a></td>
                            </form>
                        </tr>
                    @endif
                @endforeach
                    ...
                    @if ($solicitud->solicitud_status == 'aprovada' && $solicitud->pnf->user_id == Auth::user()->id)
                        <tr>
                                ...
                                @foreach ($students as $student)
                                    ...
                                    @endif
                                @endforeach
                                <td style="text-transform: capitalize;" class="text-success p-1">{{$solicitud->solicitud_status}}</td>
                                <td class="p-1"><a class="ml-1 btn btn-primary px-1 py-0" href="{{route($solicitud->solicitud_view, $solicitud)}}" style="font-size: 13px;" target='_blank'>Imprimir</a></td>
                        </tr>
                    @endif
                @endforeach
            </tbody>
